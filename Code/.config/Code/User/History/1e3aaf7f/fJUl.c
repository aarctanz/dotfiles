#include <stdio.h>
#include <stdlib.h>

typedef struct
{
    int size;
    int lineNo;
    char *content;
} Line;


typedef struct
{
    int totalLines;
    int currentLineNo;
    Line *lines;
    int capacity;
} Text;
void freeLines(int n, Line *lines);
void freeText(Text *text);

int main(int argc, char **argv)
{
    if (argc < 2)
    {
        printf("Handle empty buffer\n");
        exit(0);
    }
    FILE *fp = fopen(argv[1], "a+");
    if (fp == NULL)
    {
        perror("fopen");
        exit(EXIT_FAILURE);
    }

    // int c = 1;
    // char *line = NULL;
    // size_t len;
    // size_t n;
    // while ((n = getline(&line, &len, fp)) != -1){
    //     printf("Length: %zd, %d:", n,c);
    //     fwrite(line, n, 1, stdout);
    //     c++;
    // }
    // free(line);

    char *line = NULL;
    size_t len = 0;
    int currentLineNo = 1;

    Line *lines = (Line *)malloc(4 * sizeof(Line));
    Text text = {.totalLines = 0, .currentLineNo = 0, .lines = lines, .capacity = 4};

    while ((getline(&line, &len, fp)) != -1)
    {
        Line l = {.size = len, .lineNo = currentLineNo, .content = line};
        if (currentLineNo == text.capacity)
        {
            text.capacity *= 2;
            Line *new = realloc(text.lines, text.capacity * sizeof(Line));
            if (new == NULL)
            {
                fprintf(stderr, "Failed to allocate memory.\n");
                //  Free all the memory
                freeText(&text);
                exit(EXIT_FAILURE);
            }

            text.lines = new;
        }

        text.lines[currentLineNo - 1] = l;
        currentLineNo++;
        line = NULL;
        len = 0;
    }
    text.totalLines = currentLineNo - 1;
    free(line);

    // print all lines from text
    for (int i = 0; i < text.totalLines; i++)
    {
        printf("Line %d: %s", text.lines[i].lineNo, text.lines[i].content);
    }
    
    freeText(&text);
    fclose(fp);
    return 0;
}

void freeLines(int n, Line *lines)
{
    for (int i = 0; i < n; i++)
    {
        free(lines[i].content);
    }
    
}

void freeText(Text *text)
{
    freeLines(text->totalLines, text->lines);
    free(text->lines);
}