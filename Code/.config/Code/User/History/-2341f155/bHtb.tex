...
\begin{tikzpicture}[
    ...
    \node[on] (Suss) {$S_0(t)$};
    % \node[on] (Suss)    {g$S_0(t)$};
    % \node[on] (Inff)  [below= of Suss]    {d$S_0(t)$};
    % \node[on] (Recc)  [below= of Recc]    {s$S_0(t)$};

    % \node[on] (Suso)    {Susceptible $S(t)$};
    % \node[on] (Info) [below= of Suso] {Infectious $I(t)$};
    % \node[on] (Reco) [below= of Info] {Recovered $R(t)$};
