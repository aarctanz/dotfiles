...
const graphqlResolver = {
  ...
  Query: {
    login: async (_, { details }, context) => {
      ...
      const loginAttempt = Number(
        await redisClient.get(`LoginAttempt::${userDetails.email}`)
      );
      if (details != undefined) {
        if (loginAttempt >= 3) {
          throw new APIError({
        ...
        if (userDetails.password != details.password) {
          await redisClient.set(
            `LoginAttempt::${userDetails.email}`,
            `${loginAttempt + 1}`,
            { EX: 60 * 30 }
          );
          throw new APIError({
    ...
    sendOTP: async (_, { details }) => {
      ...
      await redisClient.set(`OTPRequest::${emailID}`, JSON.stringify(otpInfo), {
        EX: 60 * 30,
      });
      await sendMail(
        emailID,
        isNew ? "Welcome User !!" : "Forget Passowrd !!",
        `Your OTP is :- ${otpInfo.otp} !!!`
      );
      return true;
    },
    ...
    getComments: async (_, { details }, context) => {
      ...
      const result = await discussionModel
        .aggregate([
          firstMatchPipeLine,
          {
            $lookup: {
                ...
                $cond: {
                  ...
                  then: "reset",
                    ...
                    $cond: {
                      if: {
                        $eq: [
                          { $first: "$childrenComments.voteData.isUpvoted" },
                          true,
                        ],
                      },
                      then: "upvote",
          ...
          // WorkAround For Fixture No Child Comment Bug !!
          {
            $set: {
              childrenComments: {
                $cond: {
                  if: {
                    $gt: [{ $ifNull: ["$childrenComments._id", null] }, null],
                  },
    ...
    getFeatureProperty: async (_, { details }, context) => {
      ...
      const searchResults = await featureProperty
        .find({ value: { $regex: `^${searchText}`, $options: "i" }, featureID })
        .lean();
    ...
    ModgetPendingParts: async (_, { userID }, context) => {
      ...
      return (
        await plantParts.find(searchQuery).lean().populate("createdBy", "name")
      ).map((eachPart) => ({
      ...
      return (
        await contributionModel
          ...
          .populate([
            ...
            { path: "contributedBy", select: "name" },
          ])
      ).map((eachContribution) => ({
  ...
  Mutation: {
    signup: async (_, { details }) => {
      ...
      const totalAttempt = Number(
        await redisClient.get(`OTPAttempt::${details.email}`)
      );
      const otpInfo = JSON.parse(
        await redisClient.get(`OTPRequest::${details.email}`)
      );
      ...
      if (totalAttempt >= 3) {
        throw new APIError({
          code: 404,
          message:
            "You Have Exhausted Attempt Request, Try After 30 Minutes !!",
        });
      }

      if (otpInfo.otp != details.otp) {
        await redisClient.set(
          `OTPAttempt::${details.email}`,
          totalAttempt + 1,
          { EX: 60 * 30 }
        );
        throw new APIError({
    ...
    forgetPassword: async (_, { details }) => {
      ...
      const totalAttempt = Number(
        await redisClient.get(`OTPAttempt::${emailID}`)
      );
      const otpInfo = JSON.parse(
        await redisClient.get(`OTPRequest::${emailID}`)
      );
      ...
      await redisClient.set(`OTPAttempt::${emailID}`, totalAttempt + 1, {
        EX: 60 * 30,
      });
      throw new APIError({
        code: 404,
    ...
    deletePost: async (_, { postID }, context) => {
      ...
      throw new APIError({
        code: 501,
        message: "This Route Is Still In Building Phase !!",
      });
    },
    ...
    addPlantPart: async (_, { partName }, context) => {
      ...
      const grabPartData = await plantParts
        .findOne({
          name: partName,
        })
        .lean();
      ...
      if (
        grabPartData != undefined &&
        grabPartData.createdBy == context.userID
      ) {
        throw new APIError({
          code: 400,
    ...
    addPartFeature: async (_, { details }, context) => {
      ...
      const grabFeature = await partsFeatures
        .findOne({
      ...
      if (
        grabFeature != undefined &&
        (!grabFeature.isPending || grabFeature.createdBy == context.userID)
      ) {
        throw new APIError({
          code: 400,
    ...
    addContribution: async (_, { details }, context) => {
      ...
      const grabFeature = await featureProperty
        .findOne({
    ...
    updateVote: async (_, { details }, context) => {
      await protectMe(context);
      const isCommentExists = await discussionModel.exists({
        _id: details.commentID,
      });
      ...
      const grabCurrentVotesInfo = await voteModel.findOne({
        postedBy: context.userID,
      ...
      if (details.action == "reset") {
        if (grabCurrentVotesInfo == null)
          throw new APIError({
            code: 404,
            message: "You Dont Have Any Vote Yet !!",
          });

        const decrementField = grabCurrentVotesInfo.isUpvoted
          ? "upvotes"
          : "downvotes";
      ...
      if (grabCurrentVotesInfo == null) {
        ...
        const needToChange =
          (grabCurrentVotesInfo.isUpvoted && details.action == "downvote") ||
          (!grabCurrentVotesInfo.isUpvoted && details.action == "upvote");
        if (needToChange) {
